	...
	class Program
	{
		...
		static async Task Main(string[] args)
		{
			...
			if (noDevices >= 0) {
				...
				while (true) {
					var (keys, readErr) = WootingAnalogSDK.ReadFullBuffer(500);
					
					if (readErr == WootingAnalogResult.Ok)
					{
						var currentTime = DateTimeOffset.UtcNow;
						long unixTimestampMillis = currentTime.ToUnixTimeMilliseconds();

                        long unixTimestampMicros = (unixTimestampMillis * 1000) + ((currentTime.Ticks % TimeSpan.TicksPerSecond) / 10);
						...
						foreach (var analog in keys)
						{
							Console.Write($"{unixTimestampMicros} : {analog.Item1},{analog.Item2}\n");
							
							// Append the data to the CSV file
							using (StreamWriter sw = File.AppendText(fileName)) {
								sw.WriteLine($"{unixTimestampMicros},{analog.Item1},{analog.Item2}");
							}
